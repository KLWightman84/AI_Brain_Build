...
| Status | Pi initialization, prompt, repeat-stability, and oversized-context recovery passed; restart and service gates remain |
...
1. Test clean manual restart, then a systemd-managed restart and real reboot.
2. Add the loopback-only DAWN service endpoint and test non-streaming and streaming requests.
