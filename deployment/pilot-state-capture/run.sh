#!/usr/bin/env bash
# Read-only strict-allowlist capture of the verified Stage-3 WebUI/TTS/RKLLM pilot.
...
printf '%s\n' \
    '# Stage-3 WebUI/TTS/RKLLM pilot capture' \
    '' \
    ...
    '' \
    'Excluded: models, virtual environments, databases, user/runtime state, recordings, logs, caches,' \
...
for patch_file in dawn-stage3-webui-feature-gating.patch dawn-stage3-tts-text-cleanup-gate.patch dawn-stage3-tts-private-espeak-path.patch
do
...
if grep -RniE '(api[_-]?key|password|secret|authorization|cookie|credential|access[_-]?token|refresh[_-]?token|github[_-]?token)[^=]*=|gh[pousr]_[[:alnum:]_]{20,}|DAWN-[A-Z0-9-]{10,}' "$STAGE/config" "$STAGE/systemd"
then
