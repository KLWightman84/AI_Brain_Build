    ...
    def __call__(self, result: ctypes.POINTER(RKLLMResult), state: int) -> int:
        if result:
            self.generated_tokens = max(
                self.generated_tokens, int(result.contents.perf.generate_tokens)
            )
...
class RKLLMBackend:
    ...
    def _record_finish_reason(self, max_new_tokens: int) -> None:
        self._last_finish_reason = (
            "length"
            if max_new_tokens > 0 and self._collector.generated_tokens >= max_new_tokens
            else "stop"
        )
    ...
    def complete(self, prompt: str, max_new_tokens: int) -> str:
        with self._lock:
            ...
            self._record_finish_reason(max_new_tokens)
...
def _completion_response(
    model_id: str, content: str, finish_reason: str = "stop"
) -> dict[str, object]:
    now = int(time.time())
    return {
...
def create_app(backend: CompletionBackend) -> Flask:
    ...
    def chat_completions() -> Response:
        ...
        if not stream:
            try:
                content = backend.complete(prompt, max_new_tokens)
                return jsonify(
                    _completion_response(
                        backend.model_id,
                        content,
                        getattr(backend, "finish_reason", "stop"),
                    )
                )
                        ...
                        "choices": [
                            {
                                "index": 0,
                                "delta": {},
                                "finish_reason": getattr(backend, "finish_reason", "stop"),
                            }
                        ],
                    }
                )
